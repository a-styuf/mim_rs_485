...
def interface_test(mim_dev, id_list, lock: Lock):
    ...
    for id in id_list:
        ...
        if rx_frame:
            ...
            lock.acquire(), print(mim.get_time(), f"\t{mim_dev.alias}: rx_error t={(time.perf_counter() - transaction_start):.3f}"), lock.release()
            error_cnt += 1
    lock.acquire(), print(mim.get_time(), f"\t{mim_dev.alias}: rx_error_cnt {error_cnt}"), print(mim.get_time(), f"\t{mim_dev.alias}: finish <{time.perf_counter() - start_time}>"), lock.release()
    return
...
if __name__ == "__main__":
    ...
    mim_mfr = mim.MimRS485Device(alias="MFR", port="COM30", debug=True)
    mim_ma.open_id()
    mim_ma.debug = False
    mim_mfr.open_port()
    mim_mfr.debug = False
