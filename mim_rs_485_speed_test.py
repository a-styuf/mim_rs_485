...
def interface_test(mim_dev, id_list, lock: Lock, work_time: float, list_interval: float, cmd_interval: float, parsing=False):
    ...
    while (time.perf_counter() - start_time) < work_time:
        ...
        for id in id_list:
            ...
            if rx_frame:
                # lock.acquire(), logger.info(f"\t{mim_dev.alias}: rx_data {len(rx_frame)} (id={id.id}) t={(time.perf_counter() - transaction_start):.3f}: {(rx_frame.hex(' ').upper())}; id <{mim_dev.check_frame(fr=rx_frame)}>"), lock.release()
        ...
        num += 1
        lock.acquire(), logger.info(f"\t{mim_dev.alias}:\t <{(time.perf_counter() - start_time):.3f}>s, num<{num:03d}>, transaction<{transaction_cnter:03d}> rx_error_cnt {error_cnt}"), lock.release()
        # if error_cnt > 100:
        #     break
...
if __name__ == "__main__":
    ...
    print_lock = Lock()
    t_ma = Thread(target = interface_test, args = (interface_list[0], tmi_rd_100_times_list, print_lock, 5.0, 0.0, 0.0), daemon=True)
    t_ma.start()
    t_ma.join()
    #
